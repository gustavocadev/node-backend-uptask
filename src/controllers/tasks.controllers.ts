...
import { ProjectType, RequestWithUser, TaskType, TaskTypeWithProject } from '../types/types'
...
const createTask = async (req: RequestWithUser<TaskType>, res: Response) => {
  ...
  // project: es el id
  // buscamos en base de datos en la coleccion de de projects, si escuentra un project con el id del project que me estan pasando
  ...
  if (existsProject.author.toString() !== req.user!._id.toString()) {
    return res.status(403).send({ message: 'You are not the author of this project' })
  }
  ...
  try {
    ...
    // en el array de tasks de project model, agregamos el id de la tarea creada
...
const getTask = async (req: RequestWithUser<{}, { id: string }>, res: Response) => {
  const { id } = req.params

  try {
    const task: TaskTypeWithProject = await TaskModel.findById(id).populate('project').lean()
...
const updateTask = async (req: RequestWithUser<TaskType, { id: string }>, res: Response) => {
  const { id } = req.params
  ...
  const task = (await TaskModel.findById(id).populate('project')) as TaskTypeWithProject & Document
...
const deleteTask = async (req: RequestWithUser<{}, { id: string }>, res: Response) => {
  const { id } = req.params
  const task = (await TaskModel.findById(id).populate('project')) as TaskTypeWithProject & Document
